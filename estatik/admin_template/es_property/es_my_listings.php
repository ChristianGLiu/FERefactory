    ...
    if(!empty($_POST['prop_id'])){
        for($i=0; $i<count($prop_ids); $i++){
            ...
            if($_POST['es_selcted_del']=='yes'){
                ...
                $wpdb->update($wpdb->prefix.'estatik_agents', array('agent_prop_quantity' 	=> $prop_count),
            ...
            if($_POST['es_selcted_copy']=='yes'){
                ...
                $wpdb->insert($wpdb->prefix.'estatik_properties', array(
                        'prop_id' 			=> $post_id,
                ...
                $prop_count = $wpdb->get_var( "SELECT COUNT(agent_id) FROM ".$wpdb->prefix."estatik_properties WHERE agent_id = ".$select_result->agent_id );
                $wpdb->update($wpdb->prefix.'estatik_agents',array(
                ...
                foreach($select_features_arr as $feature_copy ){
                    $wpdb->insert(
                        $wpdb->prefix.'estatik_properties_features',array(
        ...
        <div class="es_all_listing_search_upper">
            <form method="post" action="<?php echo admin_url()?>admin.php?page=es_my_listings">
                <div class="es_search_filter clearFix">
                    ...
                    <?php if ($agents = es_get_agents()) : $agent_id = !empty($_POST['es_agent']) ? $_POST['es_agent'] : null; ?>
                        <select name="es_agent">
                            <option value=""><?php _e( "Agents", "es-plugin" ); ?></option>
                            <?php foreach ($agents as $agent) : ?>
                                <option value="<?php echo $agent->agent_id; ?>" <?php selected($agent_id, $agent->agent_id); ?>><?php echo $agent->agent_name; ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
                </div>
                <div class="es_search_prop clearFix">
                    ...
                    <input type="button" value="<?php _e( "Reset", "es-plugin" ); ?>" onclick="window.location='admin.php?page=es_my_listings'" />
                </div>
            </form>
        </div>
            ...
            <form id="listing_actions" action="" method="post">
                ...
                <ul>
                    ...
                    if ( ! empty( $_POST['es_agent'] ) ) {
                        $where_array[] = 'agent_id = ' . $_POST['es_agent'];
                    }
                    if($es_cat != ''){
