            ...
            <div class="es_tabs_contents clearFix">
                <div id="es_basic_info" class="es_tabs_content_in clearFix">
                    <div class="new_prop_fields_wrap clearFix">
                        <div id="es_basic_info_in">
                            ...
                            <?php if ($agents = es_get_agents()) : $aid = !empty($prop_edit->agent_id) ? $prop_edit->agent_id : 0;?>
                                <div class="new_prop_field clearFix">
                                    <span><?php _e( "Agent", "es-plugin" ); ?>:</span>
                                    <select name="agent_id">
                                        <option value=""><?php _e( "Agent", "es-plugin" ); ?></option>
                                        <?php foreach( $agents as $agent ) : ?>
                                            <option value="<?php echo $agent->agent_id; ?>" <?php selected($aid, $agent->agent_id); ?>><?php echo $agent->agent_name; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            <?php else: ?>
                ...
                <div id="es_neighboarhood" class="es_tabs_content_in clearFix">
                    <div class="boxSizing es_manager_lists">
                        ...
                        <div class="es_message" id="es_neigh_message"></div>
                        <div class="es_add_newfield full clearFix">
                            <input type="text" name="es_neigh_title" id="es_neigh_title" value="text/number" onFocus="if(this.value == 'text/number') { this.value = ''; }" onBlur="if(this.value == '') { this.value = 'text/number'; }" />
                            <span class="es_field_loader" id="es_neigh_add_loader">&nbsp;</span>
                            <a href="javascript:void(0)" class="es_add_newfield_btn" onclick="es_neigh_insertion()"><?php _e( "Add new field", "es-plugin" ); ?></a>
                            <input type="hidden" value="1" id="prop_neigh" />
                        </div>
                    </div>
                </div>
