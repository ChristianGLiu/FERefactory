<?php
	...
	$es_feature_listing = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}estatik_manager_features ORDER BY feature_title ASC" );	
...
?>            
	...
	?>
		<li id="feature_<?php echo $list->feature_id?>" class="<?php if(in_array($list->feature_id,$selected_feature)) { echo 'active'; } ?>">
			<label>
			<?php if(isset($prop_feature) && $prop_feature==1) { ?>
