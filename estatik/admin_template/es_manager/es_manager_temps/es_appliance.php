<?php
	...
	$es_appliance_listing = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}estatik_manager_appliances ORDER BY appliance_title ASC" );		
...
?>            
	...
	?>
		<li id="appliance_<?php echo $list->appliance_id?>" class="<?php if(in_array($list->appliance_id,$selected_appliance)) { echo 'active'; } ?>">
			<label>
            <?php if(isset($prop_appliance) && $prop_appliance==1) { ?>
