...
function esproperties() {
    ...
    $property_slug = empty($es_settings->property_slug) ? '' : $es_settings->property_slug;
    $labels = array(
...
// function true_properties_link( $post_link, $id = 0 ){
//     $es_settings = es_front_settings();
//     $property_slug = empty($es_settings) ? '' : $es_settings->property_slug;
//     $post = get_post($id);  
//     if ( is_object( $post ) ){
//         $terms = wp_get_object_terms( $post->ID, 'properties' );
//         if( $terms ){
//             return str_replace( 'properties' , $property_slug , $post_link );
//         }
//     }
//     return $post_link;  
// }
// add_filter( 'post_type_link', 'true_properties_link', 1, 3 );
...
function estatik_scripts(){
    ...
    if(isset($_GET['page']) && $_GET['page']=='es_settings'){
        ...
        wp_enqueue_script('es_sumoselect', DIR_URL . 'admin_template/js/jquery.sumoselect.min.js' , array( 'jquery' ));
        wp_enqueue_style('es_sumoselect_css', DIR_URL . 'admin_template/css/sumoselect.css');
        wp_enqueue_script('es_gmap_settings_scripts', DIR_URL . 'admin_template/js/es_gmap_settings_scripts.js' , array( 'jquery' ));
    }
    ...
    if(isset($_GET['page']) && ($_GET['page']=='es_agents' || $_GET['page']=='es_add_new_agent')){
        wp_enqueue_script('es_manager_scripts', DIR_URL . 'admin_template/js/es_agent_script.js' , array( 'jquery' ));
    }
    if(isset($_GET['page']) && ($_GET['page']=='es_my_subscriptions')){
        wp_enqueue_script('es_manager_scripts', DIR_URL . 'admin_template/js/es_agent_script.js' , array( 'jquery' ));
    }
    wp_enqueue_script('estatik_scripts', DIR_URL . 'admin_template/js/es_admin_scripts.js',
        array( 'jquery', 'flexslider_script', 'jquery-ui-sortable' ));
...
function es_admin_inline_js(){
	...
	\n</script>";
    echo "<script type='text/javascript'>\n";
    if(isset($_GET['page']) && $_GET['page']=='es_add_new_agent'){
    ...
    if(isset($_GET['page']) && $_GET['page']=='es_add_new_property'){
        echo "
			  ...
			  function myGeocodeFirst() {
				...
				var es_adress = jQuery('#prop_street').val()+', '+ jQuery('#es_cities option:selected').text()+', '+ jQuery('#es_states option:selected').text()+', '+ jQuery('#es_country option:selected').text()+', '+ jQuery('input[name=prop_zip_postcode]').val();
...
function es_get_category_meta($cat_id, $meta_key){
    if(!$cat_id || !$meta_key){
        return false;
    }
    global $wpdb;
    $table_name = $wpdb->prefix . 'estatik_category_meta';
    $sql = $wpdb->prepare("SELECT * FROM $table_name WHERE cat_id = %d AND meta_key = %s", $cat_id, $meta_key);
...
function es_update_category_meta($cat_id, $meta_key, $value){
    if(!$cat_id || !$meta_key || !$value){
        return false;
    }
    global $wpdb;
    $isset = es_get_category_meta($cat_id, $meta_key);
    $table_name = $wpdb->prefix . 'estatik_category_meta';
...
function es_get_agents_meta($agent_id, $meta_key){
    if(!$agent_id || !$meta_key){
        return false;
    }
    global $wpdb;
    $table_name = $wpdb->prefix . 'estatik_agents_meta';
    $sql = $wpdb->prepare("SELECT * FROM $table_name WHERE agent_id = %d AND meta_key = %s", $agent_id, $meta_key);
    $query = $wpdb->get_row($sql);
    if ($query && !empty($query)) {
        return $query->meta_value;
    }
...
function es_update_agents_meta($agent_id, $meta_key, $value){
    if(!$agent_id || !$meta_key || !$value){
        return false;
    }
    global $wpdb;
    $isset = es_get_agents_meta($agent_id, $meta_key);
    $table_name = $wpdb->prefix . 'estatik_agents_meta';
    if(isset($isset) && !empty($isset)){
        $update = $wpdb->update(
            $table_name,
...
require_once("es_agents/es_agents_functions.php");
require_once("es_subscriptions/es_subscription_functions.php");
