...
jQuery(document).ready(function(e) { 
	...
	jQuery('.es_message').hover(function(){
	...
	jQuery('.es_tabs ul li a').click(function(){
		...
		// window.history.pushState("", "", "?add_new_prop&type="+type+""+id_url);
		 
		return false;
		
	});
	jQuery('.es_tabs ul li:first-of-type a').click();
	...
	jQuery('.es_media_tabs ul li a').click(function(){
		...
		window.history.pushState("", "", "?add_new_prop&type="+type+"&media="+media+""+id_url);
	...
	jQuery("#prop_price").keydown(function (e) {
		...
		jQuery(this).siblings('div.es_new_prop_error').text(''); 
        ...
        if ((e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105)) {
			...
			jQuery(this).siblings('div.es_new_prop_error').text(jQuery('#pleaseNumbersOnly').val());
...
//	formData.append('security', window.__nonce);
...
function es_prop_detail_add_new(){
		...
		if((prop_detail_add_new_title=="") || (document.getElementById("es_basic_info_Add_new_title").defaultValue == prop_detail_add_new_title)){
			jQuery("#es_basic_info_Add_new_error").addClass('error').text(jQuery("#pleasefillfield").val());
			return false;
		} 
		
		jQuery('.es_message').removeClass('es_error').text('');
		
		jQuery("#es_basic_info_in").append('<div class="new_prop_field clearFix"><label>'+prop_detail_add_new_title+':</label><input type="text" name="prop_data[\''+prop_detail_add_new_title+'\']" value=""><a href="javascript:void(0)" onclick="es_field_del(this)" class="field_del"></a></div>')
