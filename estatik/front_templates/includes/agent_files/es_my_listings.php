	...
	if(!empty($_POST['prop_id'])){
		...
		for($i=0; $i<count($prop_ids); $i++){
			...
			if($_POST['es_selcted_copy']=='yes'){
				...
				$prop_address_mail = ($select_result->prop_address!="") ? "($select_result->prop_address)" : ""; 
				$admin_email = get_option('admin_email');
				$subj = "New property #$post_id  is added.";
				$message  = "Property #$post_id $prop_address_mail is added, please check it <a href='".admin_url()."admin.php?page=es_my_listings'>here</a> and approve.";
				...
				$select_neigh = new stdClass;
				$select_neigh_arr = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'estatik_properties_neighboarhood WHERE prop_id = '.$prop_ids[$i] );	
				foreach($select_neigh_arr as $neigh_copy ){ 
					$wpdb->insert(
					$wpdb->prefix.'estatik_properties_neighboarhood',
					array(
							'neigh_id' 		=> $neigh_copy->neigh_id,
							'neigh_distance'=> $neigh_copy->neigh_distance,
							'prop_id' 		=> $lastid
						)
					);
				}
				...
				$select_video = new stdClass;
				$select_video = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'estatik_properties_meta WHERE prop_id = "'.$prop_ids[$i].'" AND prop_meta_key = "video"' );	
				$wpdb->insert(
				$wpdb->prefix.'estatik_properties_meta',
				array(	
						'prop_meta_key' 		=> $select_video->prop_meta_key,
						'prop_meta_value' 		=> $select_video->prop_meta_value,
						'prop_id' 	=> $lastid
					)
				);
				...
				$select_images = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'estatik_properties_meta WHERE prop_id = '.$prop_ids[$i].' AND prop_meta_key = "images"' );	
...
<div class="es_wrapper"> 
	...
	<?php if (es_is_enabled_subscription() && !(int)get_user_meta(get_current_user_id(), 'es_properties_num', true) ) : ?>
		<p style="margin: 10px 0;">
		<?php if ($subscription_table_page = es_get_subscription_table_page()) : ?>
			<?php _e( 'Please', 'es-plugin' ); ?> <a href="<?php echo get_permalink($subscription_table_page); ?>"><?php _e( 'update', 'es-plugin' ); ?></a> <?php _e( 'your subscription plan', 'es-plugin' ); ?>.
		<?php else : ?>
			<?php _e( 'You can\'t add new listings.', 'es-plugin' ); ?>
		<?php endif; ?>
		</p>
	<?php else: ?>
    <div class="esHead clearFix">
        <h1 class="es_mainHeading fl"><?php the_title(); ?></h1>
        <?php $add_new_url = add_query_arg( array('add_new_prop' => ''), get_permalink(get_the_ID())); ?>
        <a href="<?php echo $add_new_url ?>"><?php _e( "Add New", "es-plugin" ); ?></a>
        <a style="margin-right:10px;" href="<?php echo add_query_arg( array(
        	'page_id' => get_query_var('page_id'),
        	'import_csv' => ''
    	), home_url() ); ?>"><?php _e( "Import Csv", "es-plugin" ); ?></a>
        <!-- <a href="<?php echo $full_url?>?add_new_prop"><?php _e( "Add New", "es-plugin" ); ?></a> -->
        <!-- <a style="margin-right:10px;" href="<?php echo $full_url?>?import_csv"><?php _e( "Import Csv", "es-plugin" ); ?></a> -->
    </div>
	<?php endif; ?>
        <div class="es_all_listing_search_upper">
        	<form method="post" action="">
                ...
                <div class="es_search_prop clearFix">
                    ...
                    <input type="button" value="<?php _e( "Reset", "es-plugin" ); ?>" onclick="window.location=window.location" />
                </div>
            </form>
        </div>
    ...
    <?php if(isset($_GET['del']) && !isset($_POST['es_selcted_copy']) && !isset($_POST['es_selcted_del'])){ ?>
    ...
    <?php if(isset($_GET['added']) && !isset($_POST['es_selcted_copy']) && !isset($_POST['es_selcted_del']) && !isset($_POST['search_list'])){ ?>
        	...
        	<form id="listing_actions" action="" method="post">
                ...
                <ul>
                    <?php  
                        ...
                        $where_array[]  =	'agent_id = '.$current_user->ID;
                    ...
                    ?>			
                                ...
                                <li class="clearFix <?php if($list->prop_pub_unpub==0) {  echo 'es_unpublish'; } ?>">
                                    ...
                                    <div>
                                        <p><a href="<?php echo get_permalink($list->prop_id)?>"><?php echo $list->prop_title?></a></p>
                                    </div>
                                    <div class="hide-phone">
                                    ...
                                    <span class="es_list_edit_del">
								        ...
								        ), home_url() ); ?>"><?php _e( "Edit", "es-plugin" ); ?></a>
								        <a href="<?php echo add_query_arg( array(
								        	'page_id' => $id,
								        	'del' => $list->prop_id
							        	), home_url() ); ?>"><?php _e( "Delete", "es-plugin" ); ?></a>
                                        <!-- <a href="<?php echo home_url().'/'.$slug.'/'?>?add_new_prop&prop_id=<?php echo $list->prop_id?>"><?php _e( "Edit", "es-plugin" ); ?></a> -->
                                        <!-- <a href="<?php echo home_url().'/'.$slug.'/'?>?del=<?php echo $list->prop_id?>"><?php _e( "Delete", "es-plugin" ); ?></a> -->
                                    </span>
                                </li>
                        <?php 
                            }
