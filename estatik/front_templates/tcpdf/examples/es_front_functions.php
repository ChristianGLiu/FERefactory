...
function es_front_settings(){	
	global $wpdb;
	return $wpdb->get_row("SELECT * FROM {$wpdb->prefix}estatik_settings");
}
...
{	
	...
	echo "<script type='text/javascript'>\n 
      ...
      $latitude = empty($es_prop_single->prop_latitude) ? 0 : $es_prop_single->prop_latitude; 
      $longitude = empty($es_prop_single->prop_longitude) ? 0 : $es_prop_single->prop_longitude; 
            }
...
function estatik_front_scripts() {
	...
	if(isset($_GET['add_new_prop'])){
		...
		wp_enqueue_script('es_property_scripts', DIR_URL . 'front_templates/js/es_property_scripts.js' , array( 'jquery' ));
	}	
